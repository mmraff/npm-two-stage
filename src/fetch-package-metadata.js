...
const offliner = require('./offliner')
...
function fetchPackageMetadata (spec, where, opts, done) {
  ...
  if (!isWindows && dep.type === 'directory' && /^[a-zA-Z]:/.test(dep.fetchSpec)) {
    let err = new Error(`Can't install from windows path on a non-windows system: ${dep.fetchSpec.replace(/[/]/g, '\\')}`)
  ...
  const isOffline = !!npm.config.get('offline')
  if (isOffline && dep.type !== 'file' && dep.type !== 'directory') {
    offliner(dep, opts, function(err, localDep) {
      if (err) return logAndFinish(err)
      getManifest(localDep)
    })
  }
  else getManifest(dep)

  function getManifest(depUsed) {
    pacote.manifest(depUsed, npmConfig({
      annotate: true,
      ...
      (pkg) => {
        if (isOffline) {
          // Try to ensure that the original request data makes it into the
