...
var offliner = require('./offliner.js')
...
function add (args, where, cb) {
  ...
  function afterRealize (_cb) { return function (err, p) {
    ...
    switch (p.type) {
      ...
      case 'remote':
        ...
        mapToRegistry(p.raw, npm.config, function (err, uri, auth) {
          ...
          addRemoteTarball(p.spec, { name: p.name }, null, auth, _cb)
        })
        break
      case 'git':
      case 'hosted':
        addRemoteGit(p.rawSpec, _cb)
        break
      default:
        if (p.name) return addNamed(p.name, p.spec, null, _cb)

        _cb(new Error("couldn't figure out how to install " + spec))
    }
  }}

  cb = afterAdd(cb)

  var next = npm.config.get('offline') ?
             offliner(afterRealize, cb) : afterRealize(cb)
  realizePackageSpecifier(spec, where, next)
}
