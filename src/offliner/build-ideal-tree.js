...
Try using the package name instead, e.g:
  ...
  [_getDownloadData] (spec) {
    ...
    if (dltType != 'git') {
      ...
      const keys = gitTrackerKeys(spec)
      dlData = this.dlTracker.getData('git', keys.repo, keys.spec)
    }
    if (!dlData)
      throw new Error(`Download Tracker knows nothing about "${spec.raw}"`)
  ...
  [_fixNpaMangling] (spec) {
    ...
    if (!spec.saveSpec.endsWith(filename)) spec.saveSpec = `file:${spec.raw}`
