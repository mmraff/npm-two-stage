...
function install (where, args, cb) {
  ...
  if (npm.config.get('offline')) {
    ...
    let offlineDir = npm.config.get('offline-dir')
    if (!offlineDir) {
...
Installer.prototype.run = function (_cb) {
  ...
  chain(installSteps, function (installEr) {
    ...
    chain(postInstallSteps, function (postInstallEr) {
      ...
      // Offline stage addition
      if (npm.config.get('offline')) {
        const gitTmpRootDir = path.join(npm.tmp, gitContext.dirNames.offlineTemps)
        rimraf(gitTmpRootDir, function(rimrafErr) {
          if (rimrafErr)
            log.warn('Installer.run', 'Failed to delete tmp git-offline root')
          cb(installEr || postInstallEr, self.getInstalledModules(), self.idealTree)
        })
      }
      else
      // END Offline stage addition
        cb(installEr || postInstallEr, self.getInstalledModules(), self.idealTree)
    })
  })
  return result
