...
function add (args, where, cb) {
  ...
  function afterRealize (cb_) { return function (err, p) {
    ...
    switch (p.type) {
      ...
      case "remote":
        ...
        mapToRegistry(p.raw, npm.config, function (err, uri, auth) {
          ...
          addRemoteTarball(p.spec, {name : p.name}, null, auth, cb_)
        })
        break
      case "git":
      case "hosted":
        addRemoteGit(p.rawSpec, cb_)
        break
      default:
        if (p.name) return addNamed(p.name, p.spec, null, cb_)

        cb_(new Error("couldn't figure out how to install " + spec))
    }
  }}
