/*
  ...
  the version of reify.js in npm-two-stage/src/offline/.
...
const Node = require('@npmcli/arborist/lib/node')
const Link = require('@npmcli/arborist/lib/link')
const Shrinkwrap = require('@npmcli/arborist/lib/shrinkwrap')
...
t.before(() =>
  ...
  .then(assets => {
    ...
    Arborist = t.mock(assets.libOffliner + '/alt-arborist', {
      ...
      '@npmcli/arborist/lib/signal-handling.js':
        require('@npmcli/arborist/lib/signal-handling.js'),

      // mock the debug so we can test these even when not enabled
      '@npmcli/arborist/lib/debug.js': mockDebug,
    })
    return start()
  })
)
...
const fixture = (t, p) => require(arbFixtures + '/reify-cases/' + p)(t)
...
const newArb = (opt) => new Arborist({
  ...
  timeout: 50 * 60 * 1000, // mmraff made it longer due to local platform necessity
  ...opt,
})
...
t.test('scoped registries', async t => {
  ...
  const ArboristMock = t.mock(n2sAssets.libOffliner + '/alt-arborist', {
    ...mocks,
    pacote,
  })
...
t.test('store files with a custom indenting', async t => {
  const tabIndentedPackageJson =
    fs.readFileSync(
      resolve(arbFixturesAbsPath, 'tab-indented-package-json/package.json'),
      'utf8'
    ).replace(/\r\n/g, '\n')
  const path = t.testdir({
...
t.test('do not delete root-bundled deps in global update', async t => {
  const path = t.testdir()
  const file = resolve(arbFixturesAbsPath, 'bundle.tgz')
...
t.test('reify audit only workspace deps when reifying workspace', async t => {
  const auditFile = resolve(arbFixturesAbsPath, 'audit-nyc-mkdirp/advisory-bulk.json')
...
t.test('install stategy linked', async (t) => {
  const Arborist = require(n2sAssets.libOffliner + '/alt-arborist')
  const abbrev = resolve(__dirname,
    arbFixtures + '/registry-mocks/content/abbrev/-/abbrev-1.1.1.tgz')
  const abbrevTGZ = fs.readFileSync(abbrev)
...
t.test('offline cases', async t => {
  /*
    It only takes one offline case to get 100% coverage with the current code;
    but if the method that determines the key that goes into [_downloadMap] is
    ever changed, some other offline test case may bring a bug to light,
    especially in non-registry package cases.
  */
  const installPath = n2sAssets.fs('installPath')

  t.test('npm registry package specifed by version', async t => {
    const dlData = {
      name: 'dummy-pkg', version: '2.1.0', spec: 'greatest',
      _resolved: 'https://registry.npmjs.org/dummy-pkg/-/dummy-pkg-2.1.0.tgz'
    }
    const arb = newArb({ path: installPath, offline: true })
    arb.dltTypeMap = mockDlt.typeMap
    arb.dlTracker = mockDlt.createSync(
      resolve(__dirname,  'fixtures/data/dlpkgs')
    )
    dlData.filename = `${dlData.name}-${dlData.version}.tgz`
    arb.dlTracker.add('tag', dlData)
    const tree = await arb.reify({ add: [`${dlData.name}@${dlData.version}`] })
  ...
  t.test('github git repo spec, plain git://', async t => {
    const name = 'dummy-git-1'
    const repo = `github.com/arbitrary/${name}`
    const commit = 'abcdef0123456789abcdef0123456789abcdef01'
    const spec = `git://${repo}#${commit}`
    const npaSpec = npa(spec)
    const dlData = {
      repo, commit,
      // The spec above gets translated to the following by both pacote
      // git fetcher and arborist consistent-resolve, so we know it will be
      // found as such in the [_downloadMap]:
      _resolved: 'git+' + npaSpec.hosted.sshurl({ noCommittish: false })
    }
    dlData.filename = encodeURIComponent(`${repo}#${commit}`) + '.tgz'
    const arb = newArb({ path: installPath, offline: true })
    arb.dltTypeMap = mockDlt.typeMap
    arb.dlTracker = mockDlt.createSync(
      resolve(__dirname, 'fixtures/data/dlpkgs')
    )
    arb.dlTracker.add('git', dlData)

    const tree = await arb.reify({ add: [spec] })
  ...
  t.test('github git repo spec https with auth', async t => {
    const name = 'dummy-git-1'
    const repo = `github.com/arbitrary/${name}`
    const commit = 'abcdef0123456789abcdef0123456789abcdef01'
    const spec = `git://skizziks@${repo}#${commit}`
    const npaSpec = npa(spec)
    const dlData = {
      repo, commit,
      // When an auth value is present, the spec above gets translated to the
      // following by both pacote git fetcher and arborist consistent-resolve,
      // so we know it will be found as such in the [_downloadMap]:
      _resolved: 'git+' + npaSpec.hosted.https({ noCommittish: false })
    }
    dlData.filename = encodeURIComponent(`${repo}#${commit}`) + '.tgz'
    const arb = newArb({ path: installPath, offline: true })
    arb.dltTypeMap = mockDlt.typeMap
  ...
  t.test('github git repo spec https, no auth', async t => {
    const name = 'dummy-git-3'
    const repo = `github.com/arbitrary/${name}`
    const commit = 'abcdef0123456789abcdef0123456789abcdef01'
    const spec = `git+https://${repo}#${commit}`
    const npaSpec = npa(spec)
    const dlData = {
      repo, commit,
      // Without an auth value, pacote git fetcher never gives a git+https URL
      // like this; likewise, arborist consistent-resolve will resolve to
  ...
  t.test('bitbucket abbreviated git repo spec', async t => {
    const name = 'dummy-git'
    const repoPath = `skizziks/${name}`
    const commit = 'be3484b5a8cb14be5e324b8ec18c440e72d6bf3a'
    const spec = `bitbucket:${repoPath}#${commit}`
    const npaSpec = npa(spec)
    const dlData = {
      repo: 'bitbucket.org/' + repoPath, commit,
      _resolved: 'git+' + npaSpec.hosted.sshurl({ noCommittish: false })
    }
    ...
    const resolvedSpec = `git+${npaSpec.hosted.sshurl()}#${commit}`
    const tree = await arb.reify({ add: [spec] })
    const newChild = tree.children.get(name)
    t.hasStrict(newChild, {
      name,
  ...
  t.test('git repo spec on unrecognized host', async t => {
    const name = 'dummy-git-2'
    const repo = `gittar.org/arbitrary/${name}`
    const commit = 'abcdef0123456789abcdef0123456789abcdef01'
    const spec = `git://${repo}#${commit}`
    const dlData = {
      repo, commit,
      // When the domain is an unrecognized git host, the resolved spec is of
      // the same form as the input spec
      _resolved: spec
    }
    dlData.filename = encodeURIComponent(`${repo}#${commit}`) + '.tgz'
    const arb = newArb({ path: installPath, offline: true })
  ...
  t.test('remote URL spec', async t => {
    const name = 'dummy-remote'
