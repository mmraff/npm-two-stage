...
module.exports.addShrinkwrap = function addShrinkwrap (pkg, next) {
  ...
  readTarballJson(tarball, 'npm-shrinkwrap.json', function (er, data) {
    if (er) {
      if (er.code === 'ENOTTARBALL') {
        // pkg._shrinkwrap already set to null above.
        return next()
      }
      if (/ as JSON/.test(er.message)) {
        er.type = 'ESHRINKWRAP'
      }
      return next(er)
    }

    pkg._shrinkwrap = data
    return next(null, pkg)
  })
}

//*** Factored-out and exported for the benefit of download/fetch-remote-tarball.js
//
function readTarballJson (tbPath, filename, next) {
  validate('SSF', arguments)

  untarStream(tbPath, function (er, untar) {
    if (er) return next(er)
    var foundJson = false
    var tbFile = path.basename(tbPath)
    var re_nameMatch = new RegExp("^(?:[^\/]+[\/])" + filename.replace(/\./, '\\.') + "$")
    untar.on('entry', function (entry) {
      if (!re_nameMatch.test(entry.path)) return
      log.silly('readTarballJson', 'Found', filename, 'in', tbFile, entry.path)
      foundJson = true
      var text = ''
      ...
      entry.on('end', function () {
        untar.close()
        log.silly('readTarballJson', 'Completed reading', filename, 'in', tbFile)
        var data
        try {
          data = parseJSON(text)
        } catch (ex) {
          var er = new Error(
            'Error parsing ' + filename + ' from ' + tbFile + ' as JSON: ' +
             ex.message
          )
          return next(er)
        }
...
module.exports.readTarballJson = readTarballJson
//*** mmraff : readTarballJson **********************************************
