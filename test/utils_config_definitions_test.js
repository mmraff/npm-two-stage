...
const { join, posix, relative, resolve } = require('path')
const t = require('tap')
const mockGlobals = require('./fixtures/npmcli/mock-globals')
const tmock = require('./fixtures/npmcli/tmock')
...
const mockDefs = (mocks = {}) => tmock(t, join(n2sAssets.fs('npmLib'), 'utils/config/definitions.js'), mocks)

const isWin = (isWindows) => ({ [join(n2sAssets.fs('npmLib'), 'utils/is-windows.js')]: { isWindows } })
...
t.before(() =>
  ...
  .then(assets => {
    ...
    pkg = require(`./${assets.rootName}/npm/package.json`)//'../../../../package.json')
  })
)
t.teardown(() => rmSync(
...
t.test('detect CI', t => {
  const ciInfoPath = join(n2sAssets.fs('nodeModules'), 'ci-info')
  const defnNoCI = mockDefs({
    [ciInfoPath]: { isCI: false, name: null },
  })
  const defnCIFoo = mockDefs({
    [ciInfoPath]: { isCI: false, name: 'foo' },
  })
...
// MMR: Added to get 100% coverage
t.test('legacy-bundling not set', t => {
  const obj = { 'legacy-bundling': false }
  const flat = {}
  mockDefs()['legacy-bundling'].flatten('legacy-bundling', obj, flat)
  t.strictSame(obj, { 'legacy-bundling': false })
  t.strictSame(flat, {})
  t.end()
})
...
// MMR: Added to get 100% coverage
t.test('global-style not set', t => {
  const obj = { 'global-style': false }
  const flat = {}
  mockDefs()['global-style'].flatten('global-style', obj, flat)
  t.strictSame(flat, {})
  t.end()
...
// MMR: Added to get 100% coverage
t.test('otp not given', t => {
  const obj = { 'auth-type': 'web', otp: null }
  const flat = {}
  mockDefs().otp.flatten('otp', obj, flat)
  t.strictSame(flat, { otp: null })
  t.strictSame(obj, { 'auth-type': 'web', otp: null })
  t.end()
})
