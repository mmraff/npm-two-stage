...
// ------------------------------------------------------
// The following must be done before Config is loaded by require
const predefs = require('@npmcli/config/lib/definitions/definitions')
const Definition = require('@npmcli/config/lib/definitions/definition')

// These two private functions verbatim from definitions.js
const define = (key, def) => {
  ...
  predefs[key] = new Definition(key, def)
}
// basic flattening function, just copy it over camelCase
const _flatten = (key, obj, flatOptions) => {
...
define('dl-dir', {
  default: null,
  type: [null, String],
  usage: '--dl-dir=<path/for/downloads>',
  description: `
...
// NOW it's safe to require Config
// ------------------------------------------------------
...
class Npm {
  ...
  async #load () {
    await this.time('npm:load:whichnode', async () => {
      ...
      const node = await which(process.argv[0]).catch(/* istanbul ignore next */() => {})
