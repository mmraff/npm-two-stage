...
const Edge = require('@npmcli/arborist/lib/edge')
...
const arbFixturesAbsPath = path.resolve(__dirname, arbFixtures)
...
t.teardown(() => {
  return rimrafAsync(path.join(__dirname, testRootName))
})
...
t.test('basic stuff', t => {
  ...
  t.equal(Arborist.name, 'AltArborist')
...
t.test('workspace nodes and deps', async t => {
  const fixture = path.join(arbFixturesAbsPath, 'workspaces-shared-deps-virtual')
