...
// TODO: probably don't have to do these like pacote anymore,
// since the Agents are not exported, so we don't need private functions
...
class ItemAgent {
  constructor(spec, opts) {
    ...
    if (!this.type)
      throw new RangeError('Cannot download package of type ' + this.p.type)

    this.dlTracker = cfg.get('dlTracker')
    this.trackerKeys = { name: this.p.name, spec: this.p.rawSpec }
    this.result = { spec: this.p.raw }
    if (this.p.name) this.result.name = this.p.name  // TODO: evaluate if we really need this
  ...
  [_pacoteOpts](extra) {
    ...
    if (extra) Object.assign(newOpts, extra)
    return newOpts
  }
...
class RegistryItemAgent extends ItemAgent {
  [_processItem]() {
    ...
    .then(mani => {
      ...
      // TODO? update url.parse to URL.whatever?
      if (!mani._resolved)
      ...
      if (typeof parsedPath !== 'string' || parsedPath.trim() === '')
        throw new Error(`Unable to parse a path from _resolved field for ${spec}`)
...
class GitItemAgent extends ItemAgent {
  [_processItem]() {
    const spec = this.p.raw
    const noShrinkwrap = cfg.get('opts').noShrinkwrap
    ...
    .manifest().then(mani => {
      ...
      this.fetchKey2 = npf.makeTarballName({
        type: 'git',
        domain: this.p.hosted.domain,
        path: this.p.hosted.path(),
        commit: mani._sha
      })

      if (ItemAgent.inflight.has(this.fetchKey2) ||
          this.dlTracker.contains('git', this.trackerKeys.name, mani._sha)) {
        throw this.duplicateSpecError
      }
      ItemAgent.inflight.add(this.fetchKey2)
...
function createAgent(rawSpec, opts) {
  if (!opts) opts = {}
  const parsed = npa(rawSpec, opts.where)
...
const handleItem = (spec, opts) => createAgent(spec, opts).run()
...
function processDependencies(manifest, opts) {
  ...
  if (manifest._shrinkwrap && !cmdOpts.noShrinkwrap) {
    ...
    for (let name in shrDeps) {
      ...
      if (dep.dev && !(cmdOpts.includeDev && opts.topLevel))
        continue
    ...
    for (const item of resolvedDeps) {
      operations.push(
        handleItem(item, { shrinkwrap: true })
    ...
    if (opts.topLevel && cmdOpts.includeDev) {
