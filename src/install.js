...
function install (where, args, cb) {
  ...
  if (npm.config.get('offline')) {
    ...
    fs.stat(offlineDir, function(er, stats) {
      if (er) {
        return cb(new Error('Unavailable path for offline packages: ' + offlineDir))
      }
      if (!stats.isDirectory()) {
        return cb(new Error('Given offline path is not a directory: ' + offlineDir))
      }
      // TODO: should also test permissions

      npm.dlTracker = new DlTracker()
      npm.dlTracker.initialize(offlineDir, null, function(er) {
        if (er) return cb(er)
