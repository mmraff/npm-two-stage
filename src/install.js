...
function install (args, cb_) {
  ...
  function next(trackerInitErr) {
    if (trackerInitErr) return cb_(trackerInitErr)
  ...
  if (npm.config.get("offline")) {
    ...
    var offlineDir = npm.config.get("offline-dir")
    if (!offlineDir) {
      log.warn("install", "offline-dir not configured; using current directory")
      offlineDir = process.cwd()
    }
    fs.stat(offlineDir, function(er, stats) {
      if (er) {
        return cb_(new Error("Unavailable path for offline packages: " + offlineDir))
      }
      if (!stats.isDirectory()) {
        return cb_(new Error("Given offline path is not a directory: " + offlineDir))
      }
      // TODO: should also test permissions

      npm.dlTracker = new DlTracker()
      npm.dlTracker.initialize(offlineDir, null, next)
    })
  }
  else next()
}
